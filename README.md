...
完整 MVP 仍要加入 YouTube 公開影片、無須登入的公開社群單篇貼文與可量化的分類模型。GitHub Pages 的真實遠端部署與公開 smoke acceptance 已完成；目前尚未完成的真實來源驗收是使用 Gemini 與使用者核准公開文章的端到端流程。PDF／論文、OCR 與標籤篩選是核心 MVP 穩定後才評估的選配項目。
