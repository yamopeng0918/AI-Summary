...
`data/classifier/training.csv` 目前共有 120 筆資料，六個分類在每個批次各 10 筆：

- 批次一 60 筆已依使用者明確決定標為 `approved`。
- 批次二 60 筆仍為 `pending`，等待逐列審核；本批次不得先行納入訓練或最終評估。

審核批次二時，每一列都應依序核對公開來源頁面、`sourceTitle`、繁體中文 `text`、`label` 與 `rationale`。審核結果遵循以下規則：
...
批次二送審時，預期為 120 列、`approved` 60 筆、`pending` 60 筆，整體每類 20 筆且每個批次每類各 10 筆。審核過程可反覆執行此計數命令確認狀態。
