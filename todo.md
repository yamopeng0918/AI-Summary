...
- [x] 為每筆已發布摘要產生 OG PNG，使用官方完整 Pan-CJK Regular／Bold OTF、渲染前 fail-closed cmap 檢查與 `18`／`40`／`36` 安全排版；詳情頁 metadata、首頁卡片、分類右上／來源 footer、containment-first 圖片 resolver、記憶體有界 PNG decoder、完整 artifact gate，以及全部六張原始解析度視覺驗收均通過。
